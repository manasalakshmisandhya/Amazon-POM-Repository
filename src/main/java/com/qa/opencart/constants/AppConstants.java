package com.qa.opencart.constants;

public class AppConstants {
	public static final int DEFAULT_TIMEOUT=2;
	public static final int DEFAULT_LONG_TIMEOUT=10;

	public static final String LOGIN_PAGE_TITLE= "Amazon Sign In";
	public static final String Login_Page_URL= "https://www.amazon.com/ap/signin";

	public static final String HOME_PAGE_TITLE="Online Shopping site in India: Shop Online for Mobiles, Books, Watches, Shoes and More - Amazon.in";

	public static final int IPHONE_11_IMAGES=6;
	public static final int APRON_IMAGES=6;
	public static final int PLANT_IMAGES=4;
	public static final int POKEMON_TOY_IMAGES=4;
}
